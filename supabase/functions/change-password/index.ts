...
async function sendOTPEmail(email: string, otp: string) {
  const html = `
    ...
    <html>
      ...
      <body>
        <div class="container">
          <h1>Slate AI – Password Change</h1>
          ...
          <div class="warning">
            <strong>Security Notice:</strong> If you didn't request this, secure your account immediately.
          </div>
        </div>
      </body>
    </html>
  ...
  await client.send({
    ...
    to: email,
    subject: 'Slate AI – Password Change OTP',
    html,
  });
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'send-otp') {
      ...
      await supabase.from('password_change_requests').delete().eq('user_id', user.id);

      const { error: insertError } = await supabase.from('password_change_requests').insert({
        user_id: user.id, email: userData.email, otp_hash: otpHash, attempts_left: MAX_ATTEMPTS,
      ...
      await supabase.from('audit_logs').insert({
        action: 'PASSWORD_OTP_SENT', module: 'profile', actor_id: user.id, actor_email: userData.email, success: true,
      });

      console.log(`Password OTP sent to: ${userData.email}`);
    ...
    if (action === 'verify-and-update') {
      ...
      const { data: request } = await supabase.from('password_change_requests').select('*').eq('user_id', user.id).single();
      ...
      await supabase.from('audit_logs').insert({
        action: 'PASSWORD_UPDATED', module: 'profile', actor_id: user.id, actor_email: request.email, success: true,
      });

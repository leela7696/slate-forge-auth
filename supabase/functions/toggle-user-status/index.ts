...
serve(async (req) => {
  ...
  try {
    ...
    // Check if user has permission to edit users (status toggle is an edit operation)
    const hasPermission = await checkPermission(supabaseAdmin, payload.userId, 'User Management', 'edit');
    
    if (!hasPermission) {
      return new Response(
        JSON.stringify({ error: 'Unauthorized - You do not have permission to change user status' }),
